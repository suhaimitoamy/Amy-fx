document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderHome() {
    ...
    mainContent.innerHTML = `<section class="home-hero slide-up"><div class="eyebrow">AMY FX <span>•</span> VIP FACILITY</div><h2>Selamat datang di Amy FX</h2><p>Ruang kerja trading untuk membaca market, mencatat keputusan, dan belajar dengan lebih terarah.</p><div class="hero-status"><span class="status-dot"></span><span>Sistem siap digunakan</span><span class="hero-divider"></span><span>Data lokal &amp; gratis</span></div></section><div class="section-heading"><div><span class="section-kicker">MODUL UTAMA</span><h2>Ruang kerja trading</h2></div><span class="section-count">${projects.length} modul</span></div><div class="quick-grid slide-up">${quick.map(quickCard).join('')}</div><div class="section-heading"><div><span class="section-kicker">TERBARU</span><h2>Akses cepat</h2></div><button class="text-button" data-nav="proyek">Lihat semua</button></div><div class="project-grid compact slide-up">${projects.slice(0, 3).map(projectCard).join('')}</div>`;
  }
  ...
  function renderProfile() {
    ...
    mainContent.innerHTML = `<div class="page-header"><div><span class="section-kicker">PENGATURAN</span><h2>Profil</h2><p>Pengaturan ringan untuk perangkat ini.</p></div></div><section class="profile-card slide-up"><div class="profile-avatar">AMY</div><div><h3>Trader Amy FX</h3><p>Mode lokal • Tanpa biaya langganan</p></div><span class="status-badge">AKTIF</span></section><section class="stats-grid"><div class="stat-card"><strong>${analyses}</strong><small>Analisis Mapping</small></div><div class="stat-card"><strong>${journal}</strong><small>Catatan Jurnal</small></div><div class="stat-card"><strong>${savedCode ? '1' : '0'}</strong><small>Kode Tersimpan</small></div></section><section class="profile-list"><div class="profile-row"><span class="tool-icon">◉</span><span><strong>Data tersimpan lokal</strong><small>Riwayat dan preferensi tetap di perangkat.</small></span><span class="check-mark">✓</span></div><div class="profile-row"><span class="tool-icon">⚡</span><span><strong>Build gratis</strong><small>Siap dibangun lewat GitHub Actions.</small></span><span class="check-mark">✓</span></div><button class="profile-row danger-row" data-profile-action="clear"><span class="tool-icon">⌫</span><span><strong>Bersihkan riwayat lokal</strong><small>Tidak menghapus API key atau file aplikasi.</small></span><span class="chevron">›</span></button></section>`;
  }
