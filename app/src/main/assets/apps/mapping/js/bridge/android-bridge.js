...
export function sendTargetsToNative(){if(state.bg&&state.key.trim()&&window.Android?.startBackgroundScanner){let s=state.result?.bestSetup;if(s&&Number.isFinite(s.entryLow)&&Number.isFinite(s.entryHigh)){let upper=Math.max(Number(s.entryLow),Number(s.entryHigh)),lower=Math.min(Number(s.entryLow),Number(s.entryHigh));window.Android.startBackgroundScanner(state.key,String(upper),String(lower))}else{let r=state.result;if(r&&Number.isFinite(r.bsl)&&Number.isFinite(r.ssl)&&r.bsl>0&&r.ssl>0){window.Android.startBackgroundScanner(state.key,String(r.bsl),String(r.ssl))}}}}
