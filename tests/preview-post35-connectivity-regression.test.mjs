...
function createMarketRuntime(initialState = {}, online = true) {
  ...
  return { api: window.AmyFXIntel, window, values };
}

test('updated post-35 integration runtimes are valid JavaScript', () => {
  for (const file of [marketPath, homePath]) {
    const result = spawnSync(process.execPath, ['--check', file], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr || result.stdout);
  }
});

test('freshness never combines a new timestamp without price with an old market price', () => {
  const now = Date.now();
  const { api } = createMarketRuntime({
    mapping: { price: 4100, capturedAt: new Date(now - 12 * 60 * 1000).toISOString(), storedAt: now },
    heatmap: { updated: new Date(now).toISOString(), zones: [], storedAt: now },
    news: { updated: new Date(now).toISOString(), items: [{ title: 'Fresh headline' }], storedAt: now }
  });
  const state = api.freshness();
  assert.equal(state.label, 'STALE');
  assert.equal(state.source, 'mapping');
});

test('explicit source timestamp wins over a newer storage write time', () => {
  const now = Date.now();
  const { api } = createMarketRuntime({
    liquidity: {
      currentPrice: 4101,
  ...
  assert.equal(api.freshness().label, 'STALE');
});

test('one genuinely fresh price-bearing source is enough for a LIVE state', () => {
  const now = Date.now();
  const { api } = createMarketRuntime({
  ...
  assert.equal(state.source, 'liquidity');
