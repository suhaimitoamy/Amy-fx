...
test('Scalper Engine reads Supabase only and hides legacy engines',async()=>{
 const engine=await read('supabase/functions/scalper-engine/index.ts'),endpoint=await read('supabase/functions/scalper-setups/index.ts');
 ...
 assert.match(endpoint,/CURRENT_ENGINE_VERSION = "amyfx-preview-scalper-multidriver-v2\.0"/); assert.match(endpoint,/SCALPER_ENGINE_EXECUTION_AUTHORITY/); assert.doesNotMatch(endpoint,/IFVG Legacy/);
});
...
test('unified production build is Amy FX 2.1.1 and has no Preview artifact channel',async()=>{
 ...
 assert.match(gradle,/"com\.amyelitesuite"/); assert.match(gradle,/versionCode = .*\?: 55/); assert.match(gradle,/versionName = .*"2\.1\.1"/); assert.match(gradle,/TWELVE_DATA_API_KEY/);
 assert.match(version,/name: '2\.1\.1', code: 55/); assert.match(workflow,/AMYFX_VERSION_NAME: "2\.1\.1"/); assert.match(workflow,/AMYFX_VERSION_CODE: "55"/);
 assert.ok(Number(update.latest_version_code)<=55); assert.equal(await exists('preview-update.json'),false); assert.equal(await exists('AmyFX-Preview-latest.apk'),false);
});
