...
test('Amy FX 1.5.2 uses versionCode 43 without changing the production applicationId', () => {
...
test('published metadata is never ahead of the APK source version', () => {
  ...
  assert.ok([40, 41, 42, 43].includes(metadata.latest_version_code));
  const expected = metadata.latest_version_code === 43
    ? '1.5.2'
  ...
  assert.ok(metadata.latest_version_code <= 43);
...
test('release workflows pin the certificate and inspect v1 plus v2 structures', () => {
  ...
  assert.match(rolling, /AMYFX_VERSION_NAME: "1\.5\.2"/);
  assert.match(rolling, /AMYFX_VERSION_CODE: "43"/);

  const manual = read('.github/workflows/build-release.yml');
  assert.match(manual, /default: "1\.5\.2"/);
  assert.match(manual, /default: "43"/);

  const candidate = read('.github/workflows/stage5-apply.yml');
  assert.match(candidate, /AMYFX_VERSION_NAME: "1\.5\.2"/);
  assert.match(candidate, /AMYFX_VERSION_CODE: "43"/);
});
