...
test('Amy FX 1.5.1 uses versionCode 42 without changing the production applicationId', () => {
...
test('release workflows pin the certificate and inspect v1 plus v2 structures', () => {
  ...
  assert.match(rolling, /AMYFX_VERSION_NAME: "1\.5\.1"/);
  assert.match(rolling, /AMYFX_VERSION_CODE: "42"/);

  const manual = read('.github/workflows/build-release.yml');
  assert.match(manual, /default: "1\.5\.1"/);
  assert.match(manual, /default: "42"/);

  const candidate = read('.github/workflows/stage5-apply.yml');
  assert.match(candidate, /AMYFX_VERSION_NAME: "1\.5\.1"/);
  assert.match(candidate, /AMYFX_VERSION_CODE: "42"/);
});
