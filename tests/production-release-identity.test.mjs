...
test('Amy FX 2.1.0 keeps the public Android identity and updater channel',()=>{
 ...
 assert.match(gradle,/com\.amyelitesuite/); assert.match(gradle,/\?: 54\)/); assert.match(gradle,/\?: "2\.1\.0"/); assert.match(gradle,/main\/update\.json/);
 assert.doesNotMatch(gradle,/learningpreview|Amy FX Preview|amyfxpreview|preview-update\.json/);
 assert.match(version,/name: '2\.1\.0', code: 54/); assert.match(version,/main\/update\.json/); assert.doesNotMatch(version,/Preview|personal\/amyfx-private/);
 ...
 assert.match(workflow,/AMYFX_VERSION_NAME: "2\.1\.0"/); assert.match(workflow,/AMYFX_VERSION_CODE: "54"/); assert.match(workflow,/latest_version_code=54/); assert.match(workflow,/Verify public update manifest source/);
