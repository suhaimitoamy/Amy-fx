...
test('Amy FX 2.0.0 keeps the public Android identity and updater channel', () => {
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME: "2\.0\.0"/);
  assert.match(workflow, /AMYFX_VERSION_CODE: "51"/);
  assert.match(workflow, /releases\/download\/amyfx-latest\/AmyFX-latest\.apk/);
  assert.match(workflow, /latest_version_code': 51/);
  assert.match(workflow, /latest_version_name': '2\.0\.0'/);
