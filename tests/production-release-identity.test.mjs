...
test('Amy FX 1.5.8 keeps the production Android identity and updater channel', () => {
  ...
  assert.match(gradle, /\?: 49\)/);
  assert.match(gradle, /\?: "1\.5\.8"/);
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME: "1\.5\.8"/);
  assert.match(workflow, /AMYFX_VERSION_CODE: "49"/);
  assert.match(workflow, /releases\/download\/amyfx-latest\/AmyFX-latest\.apk/);
  assert.match(workflow, /latest_version_code': 49/);
  assert.match(workflow, /latest_version_name': '1\.5\.8'/);
