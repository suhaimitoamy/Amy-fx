...
test('Amy FX 1.5.6 keeps the production Android identity and updater channel', () => {
  ...
  assert.match(gradle, /\?: 47\)/);
  assert.match(gradle, /\?: "1\.5\.6"/);
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME: "1\.5\.6"/);
  assert.match(workflow, /AMYFX_VERSION_CODE: "47"/);
  assert.match(workflow, /releases\/download\/amyfx-latest\/AmyFX-latest\.apk/);
  assert.match(workflow, /latest_version_code': 47/);
  assert.match(workflow, /latest_version_name': '1\.5\.6'/);
