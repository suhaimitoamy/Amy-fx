...
test('Amy FX 1.5.7 keeps the production Android identity and updater channel', () => {
  ...
  assert.match(gradle, /\?: 48\)/);
  assert.match(gradle, /\?: "1\.5\.7"/);
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME: "1\.5\.7"/);
  assert.match(workflow, /AMYFX_VERSION_CODE: "48"/);
  assert.match(workflow, /releases\/download\/amyfx-latest\/AmyFX-latest\.apk/);
  assert.match(workflow, /latest_version_code': 48/);
  assert.match(workflow, /latest_version_name': '1\.5\.7'/);
