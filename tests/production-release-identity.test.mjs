...
test('Amy FX 1.5.1 keeps the production Android identity and updater channel', () => {
  ...
  assert.match(gradle, /\?: 42\)/);
  assert.match(gradle, /\?: "1\.5\.1"/);
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME: "1\.5\.1"/);
  assert.match(workflow, /AMYFX_VERSION_CODE: "42"/);
