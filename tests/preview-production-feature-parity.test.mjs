...
test('production loads the promoted execution authority after its detailed Scalper panel', () => {
  ...
  assert.ok(panel >= 0, 'detailed Scalper panel must remain loaded');
  assert.ok(authority > panel, 'execution authority must load after the Scalper panel state exists');
  assert.ok(decision > authority, 'execution decision bridge must load after authority');
});

test('production keeps the richer multi-setup Scalper detail promoted from Preview', () => {
  ...
  for (const marker of [
    ...
    'amyfx.production.scalper.permanent-history.v1'
  ]) {
    assert.ok(source.includes(marker), `production Scalper detail marker missing: ${marker}`);
  }
  assert.doesNotMatch(source, /setInterval|visibilitychange|focusHash|scrollIntoView/);
  assert.doesNotMatch(source, /amyfx\.preview\.scalper\.permanent-history\.v1/);
});
