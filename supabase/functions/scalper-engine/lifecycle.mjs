...
export function findNextOpen(candidate, { m1 = [], m15 = [] } = {}) {
  ...
  const minute = normalizeCandles(m1, 60).find(candle => candle.open_time >= signalClose && candle.open_time < signalClose + 900);
  if (minute) return { open_time: minute.open_time, price: minute.open, source: 'M1_NEXT_OPEN' };
  const candle = normalizeCandles(m15, 900).find(item => item.open_time >= signalClose);
...
export function activateCandidate(candidate, nextOpen) {
  ...
  const stop = candidate.direction === 'BUY' ? Number(candidate.stop_reference) - buffer : Number(candidate.stop_reference) + buffer;
  const risk = candidate.direction === 'BUY' ? entry - stop : stop - entry;
  if (!(risk > EPSILON)) return {
    setup: { ...candidate, status: 'INVALIDATED', recommendation_status: 'INVALID', exit_time: nextOpen.open_time, quality: { ...(candidate.quality || {}), invalidation_reason: 'LOCAL_WICK_WRONG_SIDE' } },
  ...
  const setup = {
    ...
    quality: { ...(candidate.quality || {}), entry_source: nextOpen.source || 'UNKNOWN' }
  };
...
export function advanceSetupLifecycle(inputSetup, rows, options = {}) {
  ...
  for (const candle of values) {
    ...
    if (stopHit) {
      ...
      setup = { ...setup, status: be ? 'BE_HIT' : 'SL_HIT', stop_loss: currentStop, exit_price: currentStop, exit_time: candle.close_time, result_r: be ? 0 : -1, recommendation_status: 'CLOSED' };
      events.push({ status: setup.status, price: currentStop, candle_time: candle.open_time, result_r: setup.result_r });
      break;
    }
    if (targetHit) {
      setup = { ...setup, status: 'TP_HIT', exit_price: Number(setup.target_price), exit_time: candle.close_time, result_r: 2, recommendation_status: 'CLOSED' };
    ...
    if (setup.model === 'IFVG_SCALPER' && !setup.be_armed && oneRHit) {
      setup = { ...setup, be_armed: true, status: 'BE_ACTIVE', stop_loss: Number(setup.entry_price) };
