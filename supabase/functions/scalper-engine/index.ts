...
async function loadActiveSetups() {
  const params = new URLSearchParams({
    select: "*",
    status: `in.(${NON_TERMINAL_STATUSES.join(",")})`,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (const candidate of candidates) {
      const created = await insertSetup(candidate);
      if (!created) continue;
      inserted += 1;
      const freshEnough = nowSeconds - Number(candidate.signal_candle_close_time) <= NOTIFICATION_AGE_SECONDS;
      if (await insertEvent(created, { status: "WAITING_NEXT_OPEN", price: null, candle_time: candidate.signal_candle_open_time, result_r: null }, freshEnough)) lifecycleEvents += 1;
    }
    ...
    for (const current of active) {
      ...
      if (setup.status === "WAITING_NEXT_OPEN") {
        ...
        if (nextOpen) {
          ...
          if (activatedResult.event && await insertEvent(setup, activatedResult.event,
            nowSeconds - Number(setup.signal_candle_close_time) <= NOTIFICATION_AGE_SECONDS)) lifecycleEvents += 1;
      ...
      if (setup.status === "ACTIVE" || setup.status === "BE_ACTIVE") {
        ...
        for (const event of advanced.events) if (await insertEvent(setup, event, true)) lifecycleEvents += 1;
      }
    }
