...
async function acquireRun(nowSeconds) {
  ...
  const rows = await rest("amyfx_preview_scalper_runs?on_conflict=run_bucket", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Prefer: "resolution=ignore-duplicates,return=representation",
    },
    body: JSON.stringify({ run_bucket: bucket, started_at: new Date().toISOString() }),
  });
  return Array.isArray(rows) && rows.length > 0 ? rows[0] : null;
}

async function finishRun(bucket, payload, error = null) {
  await rest(`amyfx_preview_scalper_runs?run_bucket=eq.${bucket}`, {
    method: "PATCH",
...
async function refreshMarketSeries(interval, outputsize) {
  ...
  const response = await fetch(`${SUPABASE_URL}/functions/v1/${MARKET_FUNCTION}?${params.toString()}`, {
    headers: { Accept: "application/json" },
  });
  const text = await response.text();
  let payload;
  try { payload = JSON.parse(text); }
  catch (_) { payload = { raw: text.slice(0, 500) }; }
  if (!response.ok || payload?.status === "error") {
    throw new Error(`market_refresh_${interval}_${response.status}: ${payload?.message || payload?.raw || "unknown"}`);
  }
  return { interval, source: payload?.source || "unknown", latestOpenTime: payload?.latestOpenTime || null };
}

async function refreshMarketData() {
  // M1 menyegarkan quote serta agregasi M15/H1. Pemanggilan interval tertutup
  // tetap dilakukan agar history yang dibutuhkan engine tersedia walau aplikasi HP tidak dibuka.
  return Promise.all([
    refreshMarketSeries("1min", 500),
    refreshMarketSeries("15min", 700),
    refreshMarketSeries("1h", 400),
  ]);
}

async function loadCandles(timeframe, limit) {
  const params = new URLSearchParams({
    select: "symbol,timeframe,open_time,close_time,open,high,low,close,is_closed",
    symbol: "eq.XAU/USD",
...
async function insertSetup(candidate) {
  const rows = await rest("amyfx_preview_scalper_setups?on_conflict=id", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Prefer: "resolution=ignore-duplicates,return=representation",
    },
    body: JSON.stringify({ ...candidate, updated_at: new Date().toISOString() }),
  });
  return Array.isArray(rows) && rows.length > 0 ? rows[0] : null;
}
...
async function loadActiveSetups() {
  ...
  const params = new URLSearchParams({
    select: "*",
    status: `in.(${statuses})`,
    order: "signal_candle_close_time.asc",
    limit: "100",
  });
...
async function insertEvent(setup, event, notificationEligible) {
  ...
  const rows = await rest("amyfx_preview_scalper_events?on_conflict=setup_id,status", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Prefer: "resolution=ignore-duplicates,return=representation",
    ...
    body: JSON.stringify({
      ...
      candle_time: event.candle_time || null,
      price: event.price ?? null,
      result_r: event.result_r ?? null,
      message: lifecycleMessage(setup, event.status),
      notification_eligible: Boolean(notificationEligible),
      payload: {
        model: setup.model,
        direction: setup.direction,
        recommendation_status: setup.recommendation_status,
...
async function invokePush() {
  const response = await fetch(`${SUPABASE_URL}/functions/v1/${PUSH_FUNCTION}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
...
Deno.serve(async (request) => {
  ...
  if (url.searchParams.get("health") === "1") {
    return json({ ok: true, engine: ENGINE_VERSION, mode: "shadow", max_active_recommendations: 2 });
  }
  ...
  try {
    ...
    const [m15, h1, m1] = await Promise.all([
      loadCandles("M15", 700),
      loadCandles("H1", 400),
      loadCandles("M1", 500),
    ]);
    ...
    const candidates = detectScalperCandidates({
      m15,
      h1,
      nowSeconds,
    ...
    for (const candidate of candidates) {
      ...
      if (!created) continue;
      inserted += 1;
      if (await insertEvent(created, {
        status: "WAITING_NEXT_OPEN",
        price: null,
        candle_time: candidate.signal_candle_open_time,
        result_r: null,
      }, created.notification_enabled === true)) lifecycleEvents += 1;
    }
    ...
    const payload = {
      ...
      mode: "shadow",
      candles: { M15: m15.length, H1: h1.length, M1: m1.length },
    ...
    if (run?.run_bucket != null) {
      await finishRun(run.run_bucket, {}, error instanceof Error ? error.message : String(error)).catch(() => {});
    }
