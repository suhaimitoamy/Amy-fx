...
function titleFor(setup, status) {
  ...
  if (status === "WAITING_NEXT_OPEN") return `[SIMULASI] ${model} ${setup.direction} TERKONFIRMASI`;
  if (status === "ACTIVE") return `[SIMULASI] ${model} ${setup.direction} — ENTRY READY`;
  if (status === "BE_ACTIVE") return `[SIMULASI] ${model} ${setup.direction} — 1R TERCAPAI`;
  if (status === "TP_HIT") return `[SIMULASI] ${model} ${setup.direction} — TP 2R HIT`;
  if (status === "SL_HIT") return `[SIMULASI] ${model} ${setup.direction} — SL HIT`;
  if (status === "BE_HIT") return `[SIMULASI] ${model} ${setup.direction} — BREAKEVEN`;
  if (status === "TIME_EXIT") return `[SIMULASI] ${model} ${setup.direction} — TIME EXIT`;
  return `[SIMULASI] ${model} ${setup.direction}`;
}
...
Deno.serve(async (request) => {
  ...
  if (new URL(request.url).searchParams.get("health") === "1") return json({ ok: true, channel: CHANNEL_ID, previewOnly: true });
  if ((request.headers.get("authorization") || "") !== `Bearer ${SERVICE_ROLE_KEY}`) return json({ error: "unauthorized" }, 401);
  try {
    const eventQuery = new URLSearchParams({ select: "id,setup_id,status,message,event_time", notification_eligible: "eq.true", notified_at: "is.null", event_time: `gte.${new Date(Date.now() - 7200000).toISOString()}`, order: "event_time.asc", limit: "100" });
    const events = await rest(`amyfx_preview_scalper_events?${eventQuery.toString()}`) || [];
    if (!Array.isArray(events) || !events.length) return json({ ok: true, attempted: 0, sent: 0, failed: 0 });
    const ids = [...new Set(events.map(event => String(event.setup_id || "")).filter(Boolean))];
    const setupQuery = new URLSearchParams({ select: "id,model,direction,status,recommendation_status,entry_price,stop_loss,break_even_trigger,target_price,result_r,bars_elapsed", id: `in.(${ids.map(id => `"${id.replaceAll('"', '')}"`).join(",")})` });
    ...
    const [setups, devices] = await Promise.all([rest(`amyfx_preview_scalper_setups?${setupQuery}`), rest(`device_tokens?${deviceQuery}`)]);
    ...
    for (const event of events) {
      ...
      for (const device of activeDevices) {
        try {
          const body = String(event.message || "Sinyal scalping Amy FX diperbarui.").slice(0, 900);
          const title = titleFor(setup, event.status);
          const messageId = await client.send({
            token: String(device.fcm_token), notification: { title, body },
            data: { notification_type:"scalper", setup_id:String(setup.id), model:String(setup.model), direction:String(setup.direction), status:String(event.status), title, body, target_url:targetUrl, amyfx_route:"Mapping" },
          ...
          await logDelivery(Number(event.id), String(device.id), "sent", messageId, null);
          sent += 1; eventSent = true;
        } catch (error) {
          const code = String(error?.code || "unknown");
          await logDelivery(Number(event.id), String(device.id), "failed", null, `${code}: ${error instanceof Error ? error.message : String(error)}`);
