...
function recommendationPrefix(setup, status) {
  if (status !== "WAITING_NEXT_OPEN" && status !== "ACTIVE") return "";
  if (setup.recommendation_status === "RISK_LIMIT") return "RISK LIMIT · ";
  if (setup.recommendation_status === "DUPLICATE_CLUSTER") return "DUPLIKAT · ";
  return "";
}
...
function titleFor(setup, status) {
  ...
  if (status === "WAITING_NEXT_OPEN") return `${prefix}[SIMULASI] ${model} ${setup.direction} TERKONFIRMASI`;
  if (status === "ACTIVE") return `${prefix}[SIMULASI] ${model} ${setup.direction} — ENTRY READY`;
...
function bodyFor(setup, event) {
  const base = String(event.message || "Sinyal scalping Amy FX diperbarui.");
  if (event.status === "WAITING_NEXT_OPEN" || event.status === "ACTIVE") {
    if (setup.recommendation_status === "RISK_LIMIT") {
      return `Tidak direkomendasikan entry baru: batas dua setup aktif telah terisi. ${base}`.slice(0, 900);
    }
    if (setup.recommendation_status === "DUPLICATE_CLUSTER") {
      return `Sinyal terdeteksi sebagai cluster duplikat dan tidak dihitung sebagai entry baru. ${base}`.slice(0, 900);
    }
  }
...
async function logDelivery(eventId, deviceId, status, providerMessageId, error) {
  await rest("amyfx_preview_scalper_notification_logs?on_conflict=event_id,device_token_id", {
    ...
    body: JSON.stringify({
      ...
      error: error ? String(error).slice(0, 1800) : null,
...
Deno.serve(async (request) => {
  ...
  if (health) return json({ ok: true, channel: CHANNEL_ID, previewOnly: true });
  ...
  try {
    ...
    const setupQuery = new URLSearchParams({
      select: "id,model,direction,status,recommendation_status,entry_price,stop_loss,break_even_trigger,target_price,result_r,bars_elapsed",
      id: `in.(${setupIds.map(id => `"${id.replaceAll('"', '')}"`).join(",")})`,
    });
    const deviceQuery = new URLSearchParams({
      select: "id,device_id,fcm_token,enabled",
      enabled: "eq.true",
      device_id: `like.${PREVIEW_DEVICE_PREFIX}%`,
    });
    const [setups, devices] = await Promise.all([
      rest(`amyfx_preview_scalper_setups?${setupQuery.toString()}`),
      rest(`device_tokens?${deviceQuery.toString()}`),
    ]);
    ...
    for (const event of events) {
      ...
      for (const device of activeDevices) {
        try {
          ...
          const body = bodyFor(setup, event);
          const messageId = await client.send({
            token: String(device.fcm_token),
            notification: { title, body },
            data: {
              notification_type: "scalper",
      ...
      if (eventSent) {
        await rest(`amyfx_preview_scalper_events?id=eq.${event.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", Prefer: "return=minimal" },
